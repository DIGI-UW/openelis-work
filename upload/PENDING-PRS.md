...
### Addendum — 2026-08-05/08 (interactive session, second + third changesets)

Recorded here because the 2026-08-07 scheduled entry above covers the *what* but not these
diagnoses. The duplicate PR #256 table row this session first wrote was dropped in favour of
the scheduled run's row, which is more accurate (it carries the created-at timestamp and the
confirmed `test` success).

- **Both v2.2 changes originated as Casey's review questions, not as spec drift.** "Would the order entry clerk know which Culture protocol will be used?" and "we have the notification info in the test catalog, do we really need it here?" Each was checked against the adjacent specs before acting: M-04 §4.9 confirmed the bench-side Method picker existed but only inside *Change workflow*; M-01 §6 confirmed protocols are ordinary Methods under Admin → Test Catalog → Methods; M-11 and `test-catalog-completion-v2.md` confirmed three pre-existing notification configuration surfaces. Neither change was made on reasoning alone.
- **M-04 §4.9a was added to this same branch (commit `ac6fde4`), closing the hole v2.2 opened.** Removing the Step-1 culture-protocol override was only safe if M-04 carried a replacement, and it did not: the Method picker sat exclusively inside *Change workflow*, which demands a workflow type and a reclassification reason and re-instantiates the profile, breakpoint family, organism vocabulary, reflex variant and WHONET flavour. Investigating also surfaced a **second** gap v2.2 had created — AC-M03-04 promises a *"Not set — the bench will select a protocol"* state and M-04 had no way to **set** a protocol that was never set. §4.9a is therefore **Set or change**, on the Inoculation section, with AC-M04-20…25. Shipping it on the same branch means `main` never carries the spec with a known hole.
- **Candidate decision-log precedent (not appended, pending Casey).** *Derived values are displayed, not confirmed: if the system can resolve a value from the ordered test, do not ask a non-expert user to approve it — surface it read-only and put the override where the expertise is.* It has now bitten twice on M-03. Worth a D-0xx row if it recurs a third time.
- **PR-opening path is degraded — worth fixing once rather than working around each run.** (1) The connector's `create_pull_request` returned **403 Resource not accessible by integration** — the GitHub App lacks `pull_requests: write` on the org (Settings → Third-party Access → the app → Configure); same 403 as 07-28 and 07-29. (2) The CI auto-opener *did* fire but later than the skill's polling window suggests — two checks at +25 s and +45 s returned empty, and the PR existed by the time `scripts/open-pr.sh` ran and got a 422. **Lesson for the playbook: treat an empty result as *not yet*, and confirm with a 422 before falling back to the compare URL.** A branch rename undertaken on the false premise that a dot in `v2.2` broke the opener was unnecessary. (3) The auto-opener titles the PR from the **last commit on the branch**, so #256 was born titled "chore(ledger): record …", badly understating a spec change; title and body were corrected via a PAT `PATCH` from the local terminal.
- **Root cause of the `blocked` mergeable state: `PR_BOT_TOKEN` is empty or rejected.** `ci.yml` triggers on `pull_request` only; the auto-opener's own comment notes that PRs created with `GITHUB_TOKEN` do **not** cascade to other workflows, and it falls back to `GITHUB_TOKEN` when `PR_BOT_TOKEN` is unset. Result: `open-pr` was the only check on the head SHA, the required `test` check never started, and the PR sat unmergeable **with no failing check to explain why**. A PAT push did not help either, because the suppression follows the PR rather than the push. Worked around by dispatching `ci.yml` manually (it carries `workflow_dispatch`), which attached `test` to the head SHA — the run the 08-07 entry records as success. **Setting the `PR_BOT_TOKEN` secret fixes the auto-opener, the required check and auto-merge together**; same class of bug PR #169 fixed for the Pages deploy.
- **Jira: intentionally skipped throughout.** Casey scoped the session to "only the updates to the FRS and the openelis-work repo". Slugs unchanged, so no permalink goes stale.
