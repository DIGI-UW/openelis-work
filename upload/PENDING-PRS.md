...
- **PR:** connector `create_pull_request` returned **403** ("Resource not accessible by integration" — app still lacks `pull_requests:write`). Branch pushed with the PAT. One-click compare URL: https://github.com/DIGI-UW/openelis-work/compare/main...design/results-portal-c3-track-r?expand=1 — CI auto-opener + required `test` check land it on green. This branch carries this ledger entry too. **Status: OPEN** — verify MERGED next run.
