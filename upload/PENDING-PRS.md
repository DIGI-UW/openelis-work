...
| design/m-03-v22-derived-protocol | https://github.com/DIGI-UW/openelis-work/pull/256 | 2026-08-05 | **OPEN** (PR #256; verify MERGED next run) | Update-in-place, slug unchanged (`#/microbiology/m-03-order-entry-micro-hook`). **M-03 FRS v2.1 → v2.2** (OGC-789), second changeset of the day, off `88203fa`. Two fields leave the order-entry clerk, both for the same reason — a decision belongs to whoever holds the knowledge to make it, and configuration belongs wherever it is already configured. **(1) Culture Protocol becomes derived and read-only at Step 1.** It was required *and* overridable; a media/incubation recipe ("BAP + CHOC, 5 days, 35 °C") is not something reception can evaluate from a requisition that says "blood culture". It is now displayed so the clerk sees which workup starts, and the override moves to **M-04 §4.9**, where the person changing it is setting up the plates and the change is audited. Note the asymmetry that motivated this: `workflow_type` and the protocol are siblings, both derived from the ordered test — but a wrong `workflow_type` means TB interpreted against clinical MIC breakpoints and a wrong report, while a wrong protocol is caught at the bench within the hour. v2.1 exposed the harmless one as a required field and hid the dangerous one. **AC-M03-10 withdrawn.** **(2) Critical Value Notify removed entirely.** Notification is already configured in three places that predate it — the Test Catalog per-test **notify flag** (Basic Info), the Test Catalog **Alerts** section (shipped, functional), and critical **Ranges** — all feeding the existing `TestNotificationService` / `TestNotificationConfig` *Critical* nature, and **M-11** records the reuse rule verbatim ("reuse the existing critical detection … M-11 does not re-detect criticals"). Worse than duplication: M-11 Phase 1A ships **without** `critical_threshold_config` and triggers all criticals manually, so nothing in the shipping phase ever read this checkbox — and its default ("true for blood/CSF/sterile") was derivable from the specimen type the catalog already knows (D-028). **AC-M03-08 withdrawn.** Also adds **AC-M03-24** (stale-Method rule: a Case keeps the Method it was created with, shown marked inactive; new selections exclude inactive Methods — needed because D-002 deactivates rather than deletes, so an order can outlive its Method) and **AC-M03-25** (tile contains exactly five editable fields + one derived display). New **§2.3b** carries the full reasoning incl. the counter-argument (non-standard workups are served by Clinical History, already on the tile and macro-enabled; permission-gate rather than restore the field). Two i18n keys retired with an explicit do-not-ship note (Principle VII hygiene). §10 gains a **real dependency**: M-04's Method picker currently sits inside the *Change workflow* panel, so a protocol-only edit may not be reachable — if that is not addressed, v2.2 removes the Step-1 override without a working replacement. Mockup v1.1 → v1.2: read-only value + "derived" chip, notify checkbox replaced by an explanatory removed-note. Rendering verified in Chrome (42/42 divs, 2 selects, 1 checkbox, no overflow). Tests 260/260. **No Jira comment posted** — Casey scoped this session to the FRS + repo only. This branch also carries the ledger row. |

### Addendum — 2026-08-05 (interactive run, second changeset)

- **Step 0 — clean.** `git ls-remote` shows origin = `main` only (HEAD `88203fa`). The scheduled run earlier today already flipped **#254** (M-03 v2.1) to MERGED and drained its sources, so this run started from a clean ledger with nothing OPEN.
- **One update-in-place, no new artifacts.** Same two files as #254, same slug. Branch cut from `88203fa`, not from the earlier working clone — see the `design/m-03-v22-derived-protocol` row.
- **Both changes originated as Casey's review questions, not as spec drift.** "Would the order entry clerk know which Culture protocol will be used?" and "we have the notification info in the test catalog, do we really need it here?" Each was checked against the adjacent specs before acting: M-04 §4.9 confirmed an existing bench-side Method picker; M-01 §6 confirmed protocols are ordinary Methods under Admin → Test Catalog → Methods; M-11 and `test-catalog-completion-v2.md` confirmed three pre-existing notification configuration surfaces. Neither change was made on reasoning alone.
- **Candidate decision-log precedent (not appended, pending Casey).** *Derived values are displayed, not confirmed: if the system can resolve a value from the ordered test, do not ask a non-expert user to approve it — surface it read-only and put the override where the expertise is.* This has now bitten twice on M-03 (culture protocol) and generalises to any wizard field pre-filled from catalog data. Worth a D-0xx row if it recurs a third time.
- **PR-opening path is degraded — worth fixing once rather than working around each run.** Three things went wrong in sequence and each masked the next. (1) The connector's `create_pull_request` returned **403 Resource not accessible by integration** — the GitHub App still lacks `pull_requests: write` on the org (Settings -> Third-party Access -> the app -> Configure). Same 403 as 07-28, 07-29 and earlier today. (2) The CI auto-opener did fire, but **later than the skill's polling window suggests** — two checks at +25 s and +45 s both returned empty, and the PR existed by the time `scripts/open-pr.sh` ran and got a 422 "a pull request already exists". Lesson for the playbook: treat an empty result as *not yet*, and confirm with a 422 before falling back to the compare URL. (3) The auto-opener titles the PR from the **last commit on the branch**, so #256 was born titled "chore(ledger): record …" — badly understating a spec change. Title and body were corrected via a PAT `PATCH` from the local terminal.
- **Root cause of the `blocked` mergeable state: `PR_BOT_TOKEN` is empty or rejected.** `ci.yml` triggers on `pull_request` only; the auto-opener's own comment notes that PRs created with `GITHUB_TOKEN` do **not** cascade to other workflows, and it falls back to `GITHUB_TOKEN` when `PR_BOT_TOKEN` is unset. Result: `open-pr` was the *only* check run on the head SHA, the required `test` check never started, and the PR sat unmergeable with no visible failure. This is the same class of bug PR #169 fixed for the Pages deploy. **Setting the `PR_BOT_TOKEN` secret fixes the auto-opener, the required check, and auto-merge in one go** — until then every auto-opened PR needs a manual PAT push to start CI.
- **Jira: intentionally skipped again.** Slug unchanged; no permalink goes stale.
