...
| chore/ledger-reconcile-2026-08-14 | https://github.com/DIGI-UW/openelis-work/compare/main...chore/ledger-reconcile-2026-08-14?expand=1 | 2026-08-14 | **OPEN → auto-merge on green** (doc-only; connector `create_pull_request` returned 403 `Resource not accessible by integration` — the `pull_requests:write` regression persists; branch pushed with the PAT, CI auto-opener + required `test` check land it) | Doc-only ledger sync: flips PR #266 (`design/order-entry-collections-v1.4-withdrawal`) to MERGED on origin HEAD `5490a45`, records the four order-entry source drains to `upload/processed/drained-2026-08-14/`, adds the 2026-08-14 reconciliation block. No design artifacts. Verify MERGED next run. |
