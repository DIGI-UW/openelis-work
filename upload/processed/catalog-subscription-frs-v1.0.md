...
**Version:** 1.1 (addendum v1.1 appended to base v1.0 / internal rev 1.2)
**Date:** 2026-04-21
**Status:** Draft for Review
**Jira:** OGC-[TBD] (addendum tracked under terminology work stream T-01)
**Technology:** Java Spring Framework, Carbon React
**Related Modules:** Test Catalog (OGC-49), AMR Configuration (OGC-312), FHIR Integration, Lab Management Dashboard, Concept Mapping & Multi-Coding (T-01), Compliance Standards Administration (S-01, OGC-528), Vector Specimen Types & Taxonomy (V-01, OGC-555)

### Change Log

- **v1.1 (2026-04-21):** Addendum appended — extends catalog subscription to cover FHIR terminology resources (`CodeSystem`, `ValueSet`, `ConceptMap`), adds Open Concept Lab (OCL) hub-type awareness, introduces per-subscription auto-accept policy for ValueSet member changes, adds members-diff variant for set-level updates, adds offline terminology bundle seed path, renames navigation from "Catalog Subscriptions" to "Catalog & Terminology Subscriptions." See "Addendum v1.1" section below. Base v1.0 content is unchanged; all v1.1 additions are additive.
- **v1.0 (2026-03-24):** Initial spec — FHIR catalog subscription for `ActivityDefinition` and `PlanDefinition` resources with field-level diff review.
...
## Addendum v1.1 — Terminology Resources & OCL Hub Support

**Addendum Date:** 2026-04-21
**Addendum Status:** Draft for Review
**Related Specs:** Concept Mapping & Multi-Coding (T-01), Compliance Standards Administration (S-01 v1.1 addendum), Vector Specimen Types & Taxonomy (V-01 v1.4 addendum)

### A.0 Purpose

This addendum extends the base v1.0 catalog subscription mechanism from metadata-only sync (`ActivityDefinition`, `PlanDefinition`) into a general-purpose FHIR Terminology Service client. Once applied, an OpenELIS instance can subscribe to curated value sets and code systems published by Open Concept Lab (OCL), WHO, EUCAST, national reference laboratories, or any FHIR-conformant terminology server — and deliver those concepts into the local `ConceptMapping` table defined in T-01.

The addendum is **fully additive**. All v1.0 requirements, data model entities, APIs, and UI patterns remain unchanged. New behavior is opt-in per subscription.

### A.1 Expanded Resource Type Scope

**FR-A1-001:** The `CatalogSubscription.resourceTypes` set SHALL accept the following values in addition to the base-v1.0 values:
- `CodeSystem` — code system definitions (e.g., a vector trap type CodeSystem published by the OpenELIS OCL organization)
- `ValueSet` — enumerations of codes from one or more CodeSystems (e.g., "Baku Mutu PP22/2021 water-quality parameters" as a ValueSet of LOINC codes)
- `ConceptMap` — translation tables between code systems (e.g., local Baku Mutu parameter names ↔ LOINC codes)

**FR-A1-002:** When polling a subscription with `ValueSet` in its resource types, the system SHALL issue a FHIR search to `{fhirBaseUrl}/ValueSet?_summary=false&_count=100` and expand each retrieved ValueSet's membership using `ValueSet/{id}/$expand` when the ValueSet is declared as an intensional definition (rule-based, not explicit enumeration). Equivalent behavior applies to `CodeSystem` and `ConceptMap` polling against their respective FHIR endpoints.

**FR-A1-003:** Pulling any terminology resource type SHALL NOT require also subscribing to `ActivityDefinition` or `PlanDefinition`. A subscription may have any non-empty subset of the five resource types.

**FR-A1-004:** The "Resource Types" MultiSelect in the subscription add/edit form SHALL list all five resource types, grouped under two section headings: "Metadata" (ActivityDefinition, PlanDefinition) and "Terminology" (CodeSystem, ValueSet, ConceptMap).

### A.2 OCL Hub-Type Awareness

**FR-A2-001:** The system SHALL support a `catalog.hub.type` configuration key with values `FHIR_GENERIC` (default) or `OCL`. When set to `OCL`, the system interprets the configured `catalog.hub.url` as an OCL organization root URL (e.g., `https://api.openconceptlab.org/orgs/openelis-global/`).

**FR-A2-002:** When `catalog.hub.type = OCL`, the Catalog Registry Drawer (v1.0 FR-1-010) SHALL render the browse UI as a hierarchical tree: **Organizations → Collections → Versions**, with the connected organization pre-selected and expanded. Each collection version row SHALL show its OCL version identifier, release note, and release date. Subscribing to a collection-version creates a subscription pinned to that version identifier.

**FR-A2-003:** When `catalog.hub.type = FHIR_GENERIC`, the Catalog Registry Drawer SHALL render the flat table layout defined in v1.0 (Name, Publisher, Resource Types, Region, Resource Count, Last Updated). No change from v1.0 behavior.

**FR-A2-004:** The system SHALL auto-detect hub type by issuing a `GET {hubUrl}/metadata` request and inspecting the `CapabilityStatement.software.name` and `CapabilityStatement.implementation.description` fields. If "Open Concept Lab" or "OCL" appears in either field, the system SHALL suggest `hubType = OCL` to the administrator during initial hub configuration. The administrator retains final authority — the suggestion is non-binding.

**FR-A2-005:** The Hub Status Banner (v1.0 FR-1-009) SHALL display a small "OCL" or "FHIR" tag next to the hub name when hub type is known, to disambiguate behavior for administrators managing multiple hubs across deployments.

**FR-A2-006:** The system configuration SHALL expose the following new keys in addition to the v1.0 `catalog.hub.url` / `catalog.hub.name` / `catalog.defaultPollingIntervalHours`:

| Config Key | Type | Default | Description |
|---|---|---|---|
| `catalog.hub.type` | Enum | `FHIR_GENERIC` | `FHIR_GENERIC` or `OCL` — governs Registry Drawer layout and version-pinning behavior |
| `catalog.hub.oclOrganization` | String | null | When `hub.type = OCL`, the short identifier of the connected OCL organization (e.g., `openelis-global`). Auto-populated from the hub URL if parseable. |
| `terminology.seed.directory` | String | `/data/terminology/` | Filesystem path scanned for offline terminology bundle files at startup (see A.5). |

### A.3 Members-Diff Variant for ValueSet and ConceptMap Updates

**FR-A3-001:** For pending updates whose `resourceType` is `ValueSet` or `ConceptMap`, the system SHALL render a set-level diff instead of the v1.0 field-level diff. The set-level diff is called a **Members-Diff** and is presented as an Accordion-grouped panel.

**FR-A3-002:** A ValueSet Members-Diff SHALL organize concepts into three Accordion sections:

- **Added** (concepts present in the upstream ValueSet version but absent from the local cache): Each row shows Code, Display, CodeSystem, and an Accept/Reject toggle. Default decision is Accept if the subscription's `autoAcceptAdditions = true` (see A.4), otherwise Review.
- **Retired / Removed** (concepts present locally but absent upstream, OR marked `inactive` in the upstream CodeSystem): Each row shows Code, Display, CodeSystem, local usage count (number of `ConceptMapping` rows referencing this concept), and an Accept/Reject toggle. Default decision is Review regardless of auto-accept policy — retirements require explicit admin confirmation because they may orphan existing results.
- **Changed** (concept code unchanged but Display, Definition, or other descriptive fields differ): Each row shows Code, local Display, upstream Display, and an Accept/Reject toggle. Default decision follows the subscription's `autoAcceptAdditions` policy.

**FR-A3-003:** A ConceptMap Members-Diff SHALL organize mapping rows into Added / Removed / Changed Accordion sections using the `ConceptMap.group.element[]` structure. Each row shows `sourceCode → targetCode` with equivalence relationship (`equivalent`, `broader`, `narrower`, `relatedto`, `equal`).

**FR-A3-004:** The Members-Diff Accordion sections SHALL display count badges (e.g., "Added (14)", "Retired (2)", "Changed (3)") on their headers. Sections with zero changes SHALL NOT be rendered.

**FR-A3-005:** The Members-Diff SHALL reuse the v1.0 "Accept All / Reject All" bulk action buttons, scoped per Accordion section (e.g., "Accept All Added"). A global "Apply Decisions" button at the panel bottom commits all per-row decisions together, consistent with v1.0 FR-4-008.

**FR-A3-006:** Accepting an Added concept SHALL create a local cache row in the `Concept` table (defined in T-01 §5.2). Rejecting an Added concept SHALL record the rejection in the audit trail and prevent the concept from being re-proposed on subsequent polls unless its upstream version changes.

**FR-A3-007:** Accepting a Retired concept SHALL mark the local `Concept.isActive = false` but SHALL NOT delete any `ConceptMapping` rows that reference it. Downstream entities continue to carry the historical code; the concept is simply no longer offered in new-coding pickers. Rejecting a Retired concept SHALL keep the concept active locally with an administrator-override flag recorded in the audit trail.

**FR-A3-008:** Accepting a Changed concept SHALL overwrite the local `Concept.display`, `definition`, and any other mapped descriptive fields. Code and CodeSystem are never changed by an Accept — a code change is treated as a Retired + Added pair, not a Changed row.

**FR-A3-009:** For `CodeSystem` resource updates, the system SHALL treat the update as a special case of a ValueSet Members-Diff: every concept in the CodeSystem is considered a member. The diff presents Added / Retired / Changed accordions across all concepts in the CodeSystem. This allows subscribing to an entire CodeSystem (e.g., the OpenELIS TrapType CodeSystem) as a single unit.

### A.4 Per-Subscription Auto-Accept Policy

**FR-A4-001:** `CatalogSubscription` SHALL gain two new boolean fields:

- `autoAcceptAdditions` (default `false`) — when `true`, Added rows in a Members-Diff default to Accept and the diff auto-applies if no row requires Review.
- `autoAcceptRetirements` (default `false`) — when `true`, Retired rows in a Members-Diff default to Accept. **Not recommended** for any subscription delivering clinically relevant vocabulary.

**FR-A4-002:** Even when `autoAcceptAdditions = true`, the pending update record SHALL still be created (with all rows auto-decided) and passed through the apply pipeline so that audit logging and notification behavior is identical to manual flows.

**FR-A4-003:** A subscription SHALL allow independent setting of the two auto-accept fields. The add/edit subscription form SHALL display them in an **Advanced Settings** Carbon `Accordion` section collapsed by default, with inline help text clarifying that retirements may affect existing results.

**FR-A4-004:** When `autoAcceptAdditions = true` AND all rows in a Members-Diff are either Added or auto-accepted Changed rows, the pending update SHALL transition directly to `ACCEPTED` status without appearing in the Pending Updates queue for manual review. A notification entry SHALL still be written to the admin audit log listing the auto-applied changes.

**FR-A4-005:** If any row in a Members-Diff defaults to Review (including any Retired row in a subscription without `autoAcceptRetirements = true`), the entire pending update SHALL be queued for manual review — partial auto-apply is not supported. This keeps the admin decision model simple: either everything auto-applies, or the whole update is queued.

**FR-A4-006:** The Subscriptions DataTable SHALL display a small icon next to the subscription name indicating its auto-accept posture: no icon (manual review — default), a single chevron (additions auto-accepted), a double chevron (additions and retirements auto-accepted). The icon SHALL have a hover tooltip explaining the posture.

### A.5 Offline Terminology Bundle Seed

**FR-A5-001:** At application startup (fresh installation or upgrade), the system SHALL scan the `terminology.seed.directory` path (default `/data/terminology/`) for files with `.json` or `.ndjson` extensions. Each file is expected to be a FHIR R4 Bundle (`.json`) or NDJSON stream (`.ndjson`) containing `CodeSystem`, `ValueSet`, and/or `ConceptMap` resources.

**FR-A5-002:** Seed-loaded resources SHALL populate the local `Concept` and `ConceptMapping` tables defined in T-01 §5 using the same merge semantics as a catalog-subscription Accept — idempotent by canonical URL + version.

**FR-A5-003:** Seed-loaded resources SHALL be marked with `source = SEED_FILE` (alongside `source = SUBSCRIPTION` or `source = MANUAL` for other origins). This allows the admin UI to distinguish bootstrap content from subscription-delivered content.

**FR-A5-004:** If a later catalog-subscription poll retrieves an updated version of a resource previously loaded from a seed file, the subscription's pending-update flow SHALL take precedence — the seed-loaded rows are treated as the local baseline against which the upstream diff is computed. This enables the "ship with an offline snapshot, top up when online" pattern.

**FR-A5-005:** Seed loading errors (malformed JSON, unresolvable references, schema violations) SHALL be logged at WARN level with the filename, resource type, and error detail. Seed loading SHALL NOT prevent application startup; partial loads are acceptable, consistent with the S-01 compliance-seed pattern.

**FR-A5-006:** The system SHALL provide a convenience API endpoint `POST /api/v1/catalog-subscriptions/terminology-bundle/import` that accepts the same file formats and invokes the same merge pipeline at runtime. This is the manual equivalent of the seed-loader and is gated by `catalog.subscription.manage`.

### A.6 Navigation Rename

**FR-A6-001:** The admin navigation entry "Catalog Subscriptions" (v1.0) SHALL be renamed to "**Catalog & Terminology Subscriptions**" in all UI surfaces, to reflect the expanded scope. The i18n key `heading.catalogSubscription.pageTitle` default English value SHALL change from "Catalog Subscriptions" to "Catalog & Terminology Subscriptions." The URL path `Admin → Test Management → Catalog Subscriptions` remains unchanged for backward compatibility of saved admin bookmarks.

**FR-A6-002:** The breadcrumb SHALL reflect the new label: `Admin › Test Management › Catalog & Terminology Subscriptions › {Subscriptions | Pending Updates}`.

### A.7 New & Modified Data Model

**CatalogSubscription** — add fields:

| Field | Type | Required | Notes |
|---|---|---|---|
| autoAcceptAdditions | Boolean | Yes | Default false; when true, Added members in a Members-Diff auto-apply |
| autoAcceptRetirements | Boolean | Yes | Default false; when true, Retired members auto-apply (not recommended) |
| hubType | Enum | No | `FHIR_GENERIC` / `OCL` — denormalized from `catalog.hub.type` at subscription time for historical tracking |
| oclCollectionUrl | String (1024) | No | When hubType = OCL, the OCL collection canonical URL (e.g., `https://api.openconceptlab.org/orgs/openelis-global/collections/VectorTrapTypes/`) |
| oclVersionId | String (100) | No | When hubType = OCL, pinned collection version identifier |

**PendingCatalogUpdate** — add fields:

| Field | Type | Required | Notes |
|---|---|---|---|
| diffVariant | Enum | Yes | `FIELD` (v1.0 behavior for ActivityDefinition/PlanDefinition) or `MEMBERS` (new — for ValueSet/CodeSystem/ConceptMap) |
| autoAppliedAt | Timestamp | No | Set when the update was auto-applied via A.4 policy; null for manually reviewed updates |

**New entity: PendingMemberChange** (parallel to v1.0 PendingFieldChange, used when `diffVariant = MEMBERS`):

| Field | Type | Required | Notes |
|---|---|---|---|
| id | Long | Yes | Primary key |
| pendingUpdateId | Long | Yes | FK to PendingCatalogUpdate |
| changeType | Enum | Yes | `ADDED`, `RETIRED`, `CHANGED` |
| conceptSystemUri | String (1024) | Yes | CodeSystem URI |
| conceptCode | String (255) | Yes | Code value |
| conceptDisplayLocal | String (1024) | No | Local display (null for Added) |
| conceptDisplayUpstream | String (1024) | No | Upstream display (null for Retired) |
| mappingRowsAffected | Integer | No | For Retired rows, count of local ConceptMapping rows using this concept |
| decision | Enum | Yes | PENDING, ACCEPTED, REJECTED |

**System Configuration Settings** — see A.2 FR-A2-006 for the three new keys.

### A.8 New API Endpoints

| Method | Path | Description | Permission |
|---|---|---|---|
| GET | `/api/v1/catalog-subscriptions/pending-updates/{id}/members-diff` | Get members-diff for a ValueSet/CodeSystem/ConceptMap pending update | `catalog.update.review` |
| PUT | `/api/v1/catalog-subscriptions/pending-updates/{id}/member-decisions` | Submit per-row Accept/Reject decisions for a members-diff | `catalog.update.review` |
| POST | `/api/v1/catalog-subscriptions/terminology-bundle/import` | Import a FHIR Bundle or NDJSON file of terminology resources | `catalog.subscription.manage` |
| GET | `/api/v1/catalog-registry/ocl/organizations/{org}/collections` | List OCL collections for a connected OCL hub | `catalog.subscription.manage` |
| GET | `/api/v1/catalog-registry/ocl/collections/{org}/{collection}/versions` | List OCL collection versions | `catalog.subscription.manage` |

### A.9 New i18n Keys

| i18n Key | Default English Text |
|---|---|
| `heading.catalogSubscription.pageTitle` | Catalog & Terminology Subscriptions *(replaces v1.0 value)* |
| `heading.catalogSubscription.membersDiff` | Member Changes |
| `heading.catalogSubscription.membersAdded` | Added |
| `heading.catalogSubscription.membersRetired` | Retired |
| `heading.catalogSubscription.membersChanged` | Changed |
| `heading.catalogSubscription.advancedSettings` | Advanced Settings |
| `label.catalogSubscription.resourceTypeCodeSystem` | CodeSystem |
| `label.catalogSubscription.resourceTypeValueSet` | ValueSet |
| `label.catalogSubscription.resourceTypeConceptMap` | ConceptMap |
| `label.catalogSubscription.resourceTypeSection.metadata` | Metadata |
| `label.catalogSubscription.resourceTypeSection.terminology` | Terminology |
| `label.catalogSubscription.autoAcceptAdditions` | Auto-accept added concepts |
| `label.catalogSubscription.autoAcceptRetirements` | Auto-accept retired concepts (not recommended) |
| `label.catalogSubscription.autoAcceptHint` | Retirements may affect existing results and should normally be reviewed. |
| `label.catalogSubscription.hubTypeOcl` | OCL |
| `label.catalogSubscription.hubTypeFhir` | FHIR |
| `label.catalogSubscription.oclOrganization` | OCL Organization |
| `label.catalogSubscription.oclCollection` | Collection |
| `label.catalogSubscription.oclVersion` | Version |
| `label.catalogSubscription.source.subscription` | From subscription |
| `label.catalogSubscription.source.seedFile` | From seed file |
| `label.catalogSubscription.source.manual` | Manual |
| `label.membersDiff.changeType.added` | Added |
| `label.membersDiff.changeType.retired` | Retired |
| `label.membersDiff.changeType.changed` | Changed |
| `label.membersDiff.concept.code` | Code |
| `label.membersDiff.concept.display` | Display |
| `label.membersDiff.concept.codeSystem` | Code System |
| `label.membersDiff.concept.usageCount` | Local uses |
| `label.membersDiff.bulkAcceptAdded` | Accept All Added |
| `label.membersDiff.bulkAcceptChanged` | Accept All Changed |
| `label.membersDiff.bulkRejectAll` | Reject All |
| `message.catalogSubscription.autoAppliedSuccess` | Auto-applied {count} upstream concept changes. See audit log for details. |
| `message.catalogSubscription.terminologyBundleImportSuccess` | Terminology bundle imported: {codeSystems} CodeSystems, {valueSets} ValueSets, {conceptMaps} ConceptMaps loaded. |
| `message.catalogSubscription.retirementWarning` | {count} local records reference this concept. Accepting will preserve existing mappings but hide the concept from new pickers. |

### A.10 New Business Rules

**BR-A1:** A subscription's `resourceTypes` set MAY include any non-empty subset of `{ActivityDefinition, PlanDefinition, CodeSystem, ValueSet, ConceptMap}`. The polling logic issues one FHIR search per resource type in scope.

**BR-A2:** `diffVariant` on `PendingCatalogUpdate` is determined by `resourceType`: `ActivityDefinition` and `PlanDefinition` yield `FIELD`; `CodeSystem`, `ValueSet`, and `ConceptMap` yield `MEMBERS`. Mixed-variant updates do not exist.

**BR-A3:** An OCL subscription pinned to a specific collection version SHALL NOT advance its `oclVersionId` automatically. Administrators must explicitly edit the subscription and select a new version from the Registry Drawer. This ensures version-pinning semantics are preserved across polls.

**BR-A4:** Seed-loaded concepts (`source = SEED_FILE`) MAY be overwritten by an accepted subscription update. The `source` field is then updated to `SUBSCRIPTION` and the originating subscription ID is recorded.

**BR-A5:** When a ConceptMap pending update is accepted, the system SHALL update the relevant entries in the local `ConceptMapping` table per the new equivalence relationships. A ConceptMap accept never silently replaces a `ConceptMapping.isPrimary = true` entry — if the new mapping would demote a primary, the update is flagged as Review regardless of auto-accept policy.

### A.11 New Acceptance Criteria

**Functional:**

- [ ] A subscription can be created with `resourceTypes = [ValueSet]` only (no ActivityDefinition or PlanDefinition required)
- [ ] Polling a ValueSet subscription populates the Pending Updates queue with a `MEMBERS` diffVariant record when upstream membership has changed
- [ ] The Members-Diff panel renders three Accordion sections (Added / Retired / Changed) with correct counts and individual Accept/Reject toggles
- [ ] Sections with zero member changes are not rendered in the Accordion
- [ ] Accepting an Added concept creates a local `Concept` row; rejecting records the decision and prevents re-proposal
- [ ] Accepting a Retired concept sets `Concept.isActive = false` without deleting linked `ConceptMapping` rows
- [ ] Accepting a Changed concept updates the local display without touching `code` or `codeSystem`
- [ ] A subscription with `autoAcceptAdditions = true` auto-applies an Added-only members-diff without surfacing it in Pending Updates
- [ ] A subscription with `autoAcceptAdditions = true` but containing any Retired row queues the entire update for manual review (FR-A4-005)
- [ ] `catalog.hub.type = OCL` causes the Registry Drawer to render the hierarchical Organizations → Collections → Versions layout
- [ ] `catalog.hub.type = FHIR_GENERIC` renders the v1.0 flat table layout — backward-compatible
- [ ] Hub-type auto-detect via `/metadata` suggests `OCL` when CapabilityStatement mentions OCL but does not force the setting
- [ ] An OCL collection subscription pinned to a version does not advance to a newer version without explicit admin action (BR-A3)

**Offline Seeding:**

- [ ] FHIR JSON Bundles in `/data/terminology/` are loaded at startup and populate the Concept / ConceptMapping tables
- [ ] NDJSON terminology streams are loaded with one resource per line
- [ ] Seed loading is idempotent — restart does not duplicate existing concepts
- [ ] Malformed seed files log WARN and do not block startup
- [ ] `POST /api/v1/catalog-subscriptions/terminology-bundle/import` accepts a file at runtime with the same semantics as the seed loader

**Non-Functional:**

- [ ] All new UI strings use i18n keys — zero hardcoded English in the Members-Diff panel, Advanced Settings accordion, and OCL registry drawer
- [ ] Members-Diff panel renders within 2 seconds for ValueSets with up to 500 member changes
- [ ] The Registry Drawer OCL mode loads Organizations → Collections → Versions within 3 seconds for a typical OCL org (under 50 collections)

**Integration:**

- [ ] Accepted terminology resources populate rows in the T-01 `Concept` and `ConceptMapping` tables with `source` set correctly
- [ ] Accepted ValueSets from Baku Mutu subscriptions are picked up by the S-01 compliance-standard `applicableSampleTypes` resolver (via the S-01 v1.1 addendum)
- [ ] Accepted NCBI Taxonomy concepts populate the ConceptMapping rows associated with V-01 VectorSpecies entries (via the V-01 v1.4 addendum)
- [ ] The audit trail records every auto-applied update with actor = `SYSTEM/subscription-auto-accept`, the subscription ID, and per-row decision summary
