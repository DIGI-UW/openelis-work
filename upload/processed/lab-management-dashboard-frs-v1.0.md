...
**Related Modules:** Results Entry, Validation, TAT Reporting, Analyzer Integrations, Inventory, Quality Control
...
**FR-3-005:** Clicking the expand icon on a section row SHALL reveal one child row per lab unit within that section. Unit rows use the same column structure (Unit Name, Total, Pending, In Progress, Completed, Rejected). Unit rows are visually indented. Expanding or collapsing a section row does not trigger a data reload.
...
**FR-4-006:** TAT targets SHALL be sourced from the existing TAT configuration in OpenELIS. **Current OpenELIS TAT configuration is at the section level only.** Per-unit TAT targets are not currently supported in the existing configuration module. Adding per-unit TAT target configuration is a prerequisite story that must be completed before unit-level compliance rows can display meaningful target and compliance values. Until that story is complete, unit-level rows SHALL display `label.dashboard.noTarget` in the Target and Compliance columns.

**FR-4-007:** Clicking the expand icon on a section row SHALL reveal one child row per lab unit within that section. Unit rows use the same column structure (Unit Name, Target (hrs), Average (hrs), Min (hrs), Max (hrs), Compliance). Unit rows are visually indented. Expanding or collapsing a section row does not trigger a data reload.
...
2. **Per-unit TAT target configuration** — Current OpenELIS TAT configuration is at the section level only. A prerequisite story must add per-unit TAT target configuration to the existing TAT configuration module. Until complete, unit-level rows in the TAT expandable table display `label.dashboard.noTarget` in the Target and Compliance columns.
...
- [ ] TAT DataTable section-level rows show aggregate TAT statistics; compliance Tags display green (≥ 95%), warm-gray (85–94%), or red (< 85%) correctly
- [ ] Expanding a TAT section row reveals per-unit child rows with unit-level TAT statistics
- [ ] Section or unit with no configured TAT target displays Tag kind="gray" with text "No target set"
