...
**Related Modules:** Sample Type Domain Classification (S-04, OGC-538), Compliance Standards Administration (S-01, OGC-528, v1.1 addendum), Sampling Site Registry (S-02, OGC-536), **Catalog & Terminology Subscription (OGC-447 v1.1 addendum)**, **Concept Mapping & Multi-Coding (T-01)** — V-01 reuses the catalog-subscription pattern for hub-delivered reference data and consumes the T-01 ConceptMapping table for species- and group-level standardized identifiers (NCBI Taxonomy, VectorBase, GBIF, SNOMED CT).

### Change Log

- **v1.4 (2026-04-21):** Added addendum §14 integrating T-01 Concept Mapping & Multi-Coding and catalog-subscription v1.1. Three changes: (B1) optional multi-coding on `VectorSpecies` (NCBI Taxonomy, VectorBase, GBIF) via the shared `<MultiCodingPanel>`; (B2) optional multi-coding on `VectorGroup` (SNOMED CT family, NCBI family); (B3) explicit publication of the `TrapType` catalog to the OpenELIS OCL organization as a ValueSet, enabling cross-deployment reuse. Dependency note added: hub publication of vector catalogs (§13) now leverages the v1.1 addendum's CodeSystem/ValueSet resource types and OCL-aware hub configuration. No breaking changes to v1.3 schema.
...
## 14. Addendum v1.4 — Concept Mapping & OCL Integration (2026-04-21)

### 14.0 Purpose

v1.4 integrates V-01 with the T-01 Concept Mapping & Multi-Coding spec and the v1.1 addendum to the Catalog & Terminology Subscription FRS. Three narrow additions: multi-coding on `VectorSpecies`, multi-coding on `VectorGroup`, and a commitment to publish the `TrapType` catalog as an OCL-hosted ValueSet so that other OpenELIS deployments can subscribe to it. Nothing in v1.3 is removed. All additions are optional at runtime; a deployment that never loads any concepts continues to function exactly as v1.3.

### 14.1 VectorSpecies Multi-Coding (FR-V01-A1)

**FR-V01-A1-01:** `VectorSpecies` SHALL support attachment of zero or many `ConceptMapping` rows through the T-01 polymorphic table (`entityType = VECTOR_SPECIES`, `entityId = VectorSpecies.id`). Recommended code systems:

| Code System | canonical URI | Typical Use |
|---|---|---|
| NCBI Taxonomy | `https://www.ncbi.nlm.nih.gov/taxonomy` | Primary interop identifier — e.g., *Anopheles gambiae* = taxId `7165` |
| VectorBase | `https://vectorbase.org/species` | Vector-specific genomic/ecological identifier |
| GBIF | `https://www.gbif.org/species` | Biodiversity aggregation (One Health reporting) |
| SNOMED CT | `http://snomed.info/sct` | Clinical cross-coding for vector-borne disease surveillance |

**FR-V01-A1-02:** The Vector Species admin page (introduced in v1.2, §3a.3) SHALL extend each row's inline-expansion editor with a new **Codings** `Accordion` (collapsed by default) placed below the existing Vector Profile sub-section. The accordion body SHALL embed the shared `<MultiCodingPanel>` (T-01 FR-T01-030) with `entityType = "VECTOR_SPECIES"`, `entityId = species.id`, and `primaryRequired = false`.

**FR-V01-A1-03:** When a VectorSpecies carries one or more codings, the row's collapsed summary display SHALL show up to three code-system `Tag`s alongside the genus/species name (e.g., `NCBI 7165`, `VectorBase AGAM`, `GBIF 1651436`). The existing `organism_group` Tag (colored per v1.3) continues to appear on the left. An overflow counter (`+2 more`) is used when more than three codings are attached.

**FR-V01-A1-04:** The Add Species flow (FR-V01-UX-002) SHALL add a new optional step in the inline-expansion add form: **Codings** (collapsed Accordion). The new-species save SHALL persist both the `VectorSpecies` row and any codings entered in that step in the same transaction.

**FR-V01-A1-05:** FHIR outbound emission for `VectorSpecimen.species` SHALL produce a `CodeableConcept` populated from the species' multi-coding set per T-01 FR-T01-050. When no codings exist, the `CodeableConcept` SHALL emit `text` only (the species common name + scientific name) per T-01 FR-T01-053. The species FHIR emission path is implemented by the T-01 outbound pipeline — V-01 contributes only the multi-coding attachment.

**FR-V01-A1-06:** The initial catalog payload distributed by the hub (§13.3, ~40 species) SHALL include the NCBI Taxonomy ID as a primary coding for each species where one is available (40 of 40 expected — all listed species have a known NCBI taxonomy ID). VectorBase IDs SHALL be included for the Anopheles, Aedes, and Culex species where VectorBase publishes them. GBIF IDs are not included in the initial payload but MAY be added in a future minor hub release.

### 14.2 VectorGroup Multi-Coding (FR-V01-A2)

**FR-V01-A2-01:** `VectorGroup` SHALL support attachment of zero or many `ConceptMapping` rows through the T-01 table (`entityType = VECTOR_GROUP`, `entityId = VectorGroup.id`). Recommended code systems:

| Code System | Typical Use |
|---|---|
| SNOMED CT | Family/order classification — e.g., MOSQUITO → `18698007 Family Culicidae` |
| NCBI Taxonomy | Family-level taxId — e.g., MOSQUITO → `7157 Culicidae` |

**FR-V01-A2-02:** The Vector Groups admin page (v1.2, §3a.5) SHALL add a new **Codings** `Accordion` to each group's inline-expansion editor. The accordion embeds the shared `<MultiCodingPanel>` with `entityType = "VECTOR_GROUP"`, `entityId = group.id`, `primaryRequired = false`.

**FR-V01-A2-03:** The 5 system-protected default groups (v1.2, FR-V01-GRP-002) SHALL ship with their canonical SNOMED CT and NCBI Taxonomy family codings pre-populated in the hub catalog:

| VectorGroup | SNOMED CT (family) | NCBI Taxonomy (family) |
|---|---|---|
| MOSQUITO | `18698007 (Culicidae)` | `7157 (Culicidae)` |
| TICK | `84770000 (Ixodidae)` *or* `62321000 (Argasidae)` | `6939 (Ixodidae)` / `6936 (Argasidae)` |
| RODENT | (no single family — leave unmapped) | `9989 (Rodentia)` |
| OTHER_ARTHROPOD | (no single family) | `6656 (Arthropoda)` |
| SANDFLY | `84679000 (Psychodidae)` | `7198 (Psychodidae)` |

Where no single family code exists, the group's default shipped mapping SHALL be empty and administrators are encouraged to leave the group uncoded at that level.

**FR-V01-A2-04:** The collapsed row display on the Groups admin page SHALL add a single `Tag` showing the primary family coding (e.g., `SNOMED 18698007`) next to the group's color-tag label. No overflow indicator is needed at the group level — at most two codings per group are expected in practice.

### 14.3 TrapType Publication to OCL (FR-V01-A3)

**FR-V01-A3-01:** The V-01 initial TrapType catalog (§13.4, ~15 records) SHALL be published by the OpenELIS Global project as a FHIR `CodeSystem` + `ValueSet` pair to the OpenELIS organization on Open Concept Lab (OCL), at the canonical URLs:

- CodeSystem: `https://api.openconceptlab.org/orgs/openelis-global/sources/TrapTypes/`
- ValueSet: `https://api.openconceptlab.org/orgs/openelis-global/collections/VectorTrapTypes/`

**FR-V01-A3-02:** Deployments subscribing to the OpenELIS Community Hub's `VECTOR_TRAP_TYPE` catalog (v1.2 §13.1) SHALL receive trap type updates through the catalog-subscription v1.1 addendum Members-Diff pipeline (catalog-subscription §A.3). Publication of the catalog to OCL enables third-party deployments outside the Community Hub to consume the same authoritative list via the standard FHIR Terminology Service.

**FR-V01-A3-03:** The `TrapType` entity's `code` column (v1.0 §5) SHALL be treated as the member concept code for the OCL publication. A new optional `conceptUrl` column MAY be populated on TrapType to record the OCL concept URL for traceability; this is read-only to the admin UI and populated by the subscription/seed-file loader.

**FR-V01-A3-04:** V-01 does NOT introduce multi-coding on `TrapType` in this addendum. A TrapType is identified by its code within the OpenELIS authoritative catalog; cross-system identifiers (e.g., WHO equipment identifiers, manufacturer SKUs) are out of scope for v1.4 and deferred to a future minor revision if needed.

### 14.4 Data Model Additions

**VectorSpecies** — no column changes. All multi-coding state lives in the T-01 `ConceptMapping` table. Existing v1.3 columns (`ncbiTaxonId` if any was present — V-01 v1.3 does not have one) remain untouched; the multi-coding model is additive, not replacement.

**VectorGroup** — no column changes. Multi-coding lives in T-01.

**TrapType** — add optional column `conceptUrl` (String 1024, nullable). Populated by the subscription / seed loader; read-only in admin UI.

### 14.5 API Endpoint Additions

No new V-01 endpoints are introduced — all multi-coding CRUD flows through the T-01 endpoints (`/api/v1/terminology/entities/VECTOR_SPECIES/{id}/mappings`, etc.). The v1.1 addendum to catalog-subscription defines the ValueSet/CodeSystem sync endpoints used to populate the local Concept cache.

### 14.6 i18n Key Additions

| i18n Key | Default English Text |
|---|---|
| `heading.vectorSpecies.codings` | Codings |
| `heading.vectorGroup.codings` | Codings |
| `label.vectorSpecies.noCodings` | No standardized codings attached |
| `label.vectorSpecies.morecodings` | +{0} more |
| `label.trapType.conceptUrl` | OCL Concept URL |
| `message.vectorSpecies.codingAutoMapped` | NCBI Taxonomy coding attached automatically from hub catalog. |

### 14.7 New Business Rules

**BR-V01-A1:** A VectorSpecies's primary coding (if any) SHALL be NCBI Taxonomy whenever possible. The hub-delivered catalog enforces this; locally-added species MAY use any code system as primary. A validation warning (InlineNotification kind="warning", non-blocking) SHALL display if the administrator sets a non-NCBI coding as primary on a locally-created species, with the text *"NCBI Taxonomy is the recommended primary coding for vector species interop."*

**BR-V01-A2:** Removing a VectorGroup SHALL cascade-delete its ConceptMapping rows via T-01 FR-T01-006. System-protected default groups (v1.2 FR-V01-GRP-002) cannot be deleted — the existing v1.2 rule applies transitively to their codings.

**BR-V01-A3:** Subscription-delivered codings on a locally-created VectorSpecies (e.g., an admin added a new species and the hub later published an NCBI coding for it) SHALL flow through the catalog-subscription v1.1 Members-Diff review workflow (§A.3). The admin decides whether to accept the auto-attached coding; it is not applied silently.

**BR-V01-A4:** When TrapType subscription delivers an updated `display` for a code already present locally, the local `TrapType.displayName` SHALL be updated in place only if the administrator accepts the pending update. The FieldDiff review pattern from catalog-subscription v1.0 applies — not the Members-Diff pattern, because TrapType updates are field-level.

**BR-V01-A5:** Publishing the TrapType catalog to OCL is a project-level operation performed by OpenELIS Global maintainers, not an in-app action. The product SHALL document the publication process and ensure the canonical URLs referenced in FR-V01-A3-01 are stable across TrapType catalog versions.

### 14.8 Acceptance Criteria (Addendum)

#### VectorSpecies Multi-Coding

- [ ] The Vector Species admin row expansion renders a **Codings** accordion embedding `<MultiCodingPanel entityType="VECTOR_SPECIES">`
- [ ] A species with zero codings continues to work exactly as v1.3 — no regression in the existing species editor
- [ ] When three or fewer codings are attached, each renders as a Tag in the collapsed row summary; four or more show three + an overflow counter
- [ ] The hub-delivered initial catalog populates NCBI Taxonomy codings as primary on all 40 species
- [ ] VectorBase codings are populated on supported Anopheles, Aedes, and Culex species
- [ ] Setting a non-NCBI coding as primary on a locally-created species displays the warning notification (non-blocking)
- [ ] FHIR outbound `VectorSpecimen.species` emits a CodeableConcept with NCBI primary + any secondary codings; zero-coding species emit text-only

#### VectorGroup Multi-Coding

- [ ] The Vector Groups admin row expansion renders a **Codings** accordion for each group
- [ ] The 5 system-protected default groups ship with their canonical SNOMED CT and NCBI Taxonomy family codings where available (see FR-V01-A2-03 table)
- [ ] Groups with no natural family coding (RODENT, OTHER_ARTHROPOD at SNOMED level) ship with empty codings and the admin can leave them empty without error
- [ ] The collapsed row summary displays at most one family-level Tag per group

#### TrapType OCL Publication

- [ ] TrapType table gains the optional `conceptUrl` column; existing rows are nullable
- [ ] On subscription / seed load, `conceptUrl` is populated from the hub-delivered catalog payload
- [ ] The OCL canonical URL `https://api.openconceptlab.org/orgs/openelis-global/collections/VectorTrapTypes/` is reachable and publishes the 15 trap types from §13.4 (verified at project-level, not in-app)
- [ ] A third-party OpenELIS deployment can subscribe to the OCL ValueSet directly (using catalog-subscription v1.1 OCL hub type) and receive the same trap types

#### Integration

- [ ] Subscription-delivered species and group codings flow through the Members-Diff review workflow (catalog-subscription v1.1 §A.3) — they are not applied silently
- [ ] Subscription-delivered TrapType display updates flow through the FieldDiff review workflow (catalog-subscription v1.0 §A)
- [ ] A VectorSpecimen with a species that has no multi-coding still serializes cleanly to FHIR (text-only CodeableConcept per T-01 FR-T01-053)
- [ ] Audit trail records all species/group coding additions, removals, primary reassignments, and subscription-applied changes
