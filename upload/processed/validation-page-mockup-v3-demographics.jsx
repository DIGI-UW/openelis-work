...
// ═══════════════════════════════════════════════════════════════
// OpenELIS Global — Validation Configuration & Page Mockup v3.0
// v3 adds: Patient Sex & Age (D-M-Y) columns + header display
...
const calculateAgeDMY = (dobStr, collectionDateStr) => {
  if (!dobStr || !collectionDateStr) return null;
  const parts = (s) => { const [m, d, y] = s.split("/"); return new Date(+y, +m - 1, +d); };
  const birth = parts(dobStr);
  const collection = parts(collectionDateStr);
  ...
  if (days < 0) { months -= 1; days += new Date(collection.getFullYear(), collection.getMonth(), 0).getDate(); }
  if (months < 0) { years -= 1; months += 12; }
  return `${days}D-${months}M-${years}Y`;
};

const getSexShort = (sex) => {
  if (sex === "M") return t("label.validation.sex.male.short");
  if (sex === "F") return t("label.validation.sex.female.short");
  return t("label.validation.sex.unknown.short");
};

const getSexFull = (sex) => {
  if (sex === "M") return t("label.validation.sex.male");
  if (sex === "F") return t("label.validation.sex.female");
  return t("label.validation.sex.unknown");
};
...
const MOCK_RESULTS = [
  {
    ...
    sex: "M", dob: "01/11/2012", collectionDate: "03/07/2026",
    test: "WBC", sampleType: "Whole Blood",
  ...
  },
  {
    ...
    sex: "M", dob: "01/11/2012", collectionDate: "03/07/2026",
    test: "RBC", sampleType: "Whole Blood",
  ...
  },
  {
    ...
    sex: "F", dob: "03/22/1981", collectionDate: "03/08/2026",
    test: "Glucose", sampleType: "Serum",
  ...
  },
  {
    ...
    sex: "F", dob: "03/22/1981", collectionDate: "03/08/2026",
    test: "Creatinine", sampleType: "Serum",
  ...
  },
  {
    ...
    sex: "M", dob: "09/15/1963", collectionDate: "03/07/2026",
    test: "Hemoglobin", sampleType: "Whole Blood",
  ...
  },
  {
    ...
    sex: "F", dob: "07/04/1997", collectionDate: "03/08/2026",
    test: "TSH", sampleType: "Serum",
  ...
  // v3: Neonatal patient — demonstrates why age in D-M-Y matters for reference range validation
  {
    id: "r7", labNumber: "DEV01260000001238", patientId: "9998877", patientName: "Razafy, Baby",
    sex: "F", dob: "12/01/2025", collectionDate: "03/08/2026",
    test: "Bilirubin (Total)", sampleType: "Serum",
    analyzer: "Indiko Plus", rangeText: "0.1-1.2", unit: "mg/dL", result: "5.8",
    isNormal: false, flags: ["above-normal"], enteredBy: "M. Jones", enteredAt: "14:05",
    labUnit: "Chemistry", qcStatus: "pass",
    validationLevelsRequired: 1, validationLevelCurrent: 1,
    validationHistory: [],
  },
  // v3: Patient with missing DOB — demonstrates "unknown" age handling
  {
    id: "r8", labNumber: "DEV01260000001239", patientId: "5551234", patientName: "Unknown, Patient",
    sex: null, dob: null, collectionDate: "03/08/2026",
    test: "Malaria RDT", sampleType: "Whole Blood",
    analyzer: null, rangeText: "Negative", unit: "", result: "Positive",
    isNormal: false, flags: [], enteredBy: "A. Lee", enteredAt: "15:30",
    labUnit: "Serology", qcStatus: "pass",
    validationLevelsRequired: 1, validationLevelCurrent: 1,
    validationHistory: [],
  },
];
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              <th className="text-left text-xs font-semibold text-gray-600 uppercase tracking-wider px-3 py-2.5">{t("label.validation.patientSex")}</th>
              <th className="text-left text-xs font-semibold text-gray-600 uppercase tracking-wider px-3 py-2.5">{t("label.validation.patientAge")}</th>
                ...
                <Fragment key={result.id}>
                  ...
                  <tr className={`border-b border-gray-100 transition-colors cursor-pointer ${
                    ...
                    <td className="px-3 py-2.5">
                      ...
                      <div className="text-xs text-gray-500">{result.patientId} · {result.patientName}</div>
                    </td>
                    <td className="px-3 py-2.5">
                      <span className="text-sm font-bold text-gray-800">{getSexShort(result.sex)}</span>
                    </td>
                    <td className="px-3 py-2.5">
                      <span className="text-sm font-mono text-gray-700" style={{ whiteSpace: "nowrap" }}>
                        {calculateAgeDMY(result.dob, result.collectionDate) || "\u2014"}
                      </span>
                    </td>
                    <td className="px-3 py-2.5">
                  ...
                  {isExpanded && (
                    <tr>
                      <td colSpan={14} className="bg-gray-50/50 px-6 py-4 border-b border-gray-200">
                        {/* Patient Banner — v3: prominent Sex + Age (D-M-Y) display */}
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-gray-500">ID: {result.patientId} · DOB: {result.dob || "Unknown"}</div>
                              </div>
                              {/* Sex Tag */}
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                result.sex === "M" ? "bg-blue-50 text-blue-700 border border-blue-200" :
                                result.sex === "F" ? "bg-purple-50 text-purple-700 border border-purple-200" :
                                "bg-gray-100 text-gray-600 border border-gray-200"
                              }`}>
                                {getSexFull(result.sex)}
                              </span>
                              {/* Age in D-M-Y */}
                              <div className="flex items-center gap-1.5">
                                <span className="text-xs text-gray-500">{t("label.validation.patientAge")}:</span>
                                <span className="text-sm font-mono font-bold text-gray-800">
                                  {calculateAgeDMY(result.dob, result.collectionDate) || t("label.validation.age.unknown")}
                                </span>
                              </div>
                            </div>
                            <div className="text-xs text-gray-500 text-right">
                              <div>Entered by <span className="font-medium">{result.enteredBy}</span> at {result.enteredAt}</div>
                              <div className="text-gray-400">Collected: {result.collectionDate}</div>
                            </div>
                          </div>
                        </div>
