...
Read **`references/suite-catalog.md`** for the suite inventory, phases, and route-discovery patterns. (Admin routes: use the canonical `admin-ia-inventory.md` in the `openelis-design` skill, not a frozen copy.) Scan the outputs folder for prior reports.
...
| `references/playwright-harness.md` | When using the Playwright harness or Carbon component workarounds; operational hazards; where the harness repo lives |
