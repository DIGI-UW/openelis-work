...
> **Where the harness lives:** the Playwright specs (`tests/chains/*.spec.ts`,
> `tests/personas/*`), helpers (`helpers/networkCapture.ts`, `helpers/apiShapes.ts`,
> `helpers/seed-*.ts`), and `seed-data.setup.ts` live in the QA automation test repo —
> **TODO: record the exact repo + path here.** They do NOT ship inside this skill bundle.
> Canonical spec layout is **one spec per chain/persona** (`tests/chains/chain-a-*.spec.ts`,
> run via `--project=chain-a`); the older single `openelis-e2e.spec.ts` layout is legacy.
