...
| ORD-10 | **P0** | When a program is selected from the Program dropdown, an 'Additional Order Information' section shall display program-specific fields configured for that program. The section appears/disappears dynamically based on the selected program. These fields provide custom context for specific workflows (e.g., VL Program shows ARV regimen, duration on ARV, indication for VL test, pregnancy status, last VL result; Water Quality Monitoring shows water source type, sampling point, regulatory standard, field measurements). For labs where a program is commonly used (e.g., VL Program), it may be pre-selected by default. | Selecting a program dynamically shows the correct additional fields. Changing program hides previous and shows new fields. Fields are configurable per program in admin settings. Data saves to the order and is available in subsequent steps. |
