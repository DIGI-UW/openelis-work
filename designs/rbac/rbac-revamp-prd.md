...
| `order:cancel_test` | Cancel Test (no results) | `/SampleManagement` (action) | Reception |
| `order:cancel_test_with_results` | Cancel Test (with results) | `/SampleManagement` (action) | Global Administrator |
...
- Some permissions are **action-level** rather than screen-level (e.g., `order:cancel_test` and `order:cancel_test_with_results`). These gate specific actions within a screen rather than access to the screen itself. The Cancel Test button in Sample Management is only shown if the user has the appropriate cancel permission. Cancelling a test that already has results requires `order:cancel_test_with_results` (a higher-privilege permission, mapped to Global Administrator by default) to protect data integrity and audit compliance.
...
- The Create/Edit Role form displays a **sticky permission counter bar** showing the number of selected permissions out of the total (e.g., "12 of 74 permissions selected") that updates in real-time as checkboxes are toggled. The Save and Cancel actions are embedded in this sticky bar for constant visibility.
