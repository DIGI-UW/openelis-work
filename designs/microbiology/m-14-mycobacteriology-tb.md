...
Records decontamination/concentration of the raw specimen into the processed aliquot: method, reagent lot (M-12 — e.g. NALC-NaOH), date/time, processed by. Saving writes the row + an AUTO timeline event. This is the prerequisite aliquot that smear, molecular, and culture are set up from. The processed aliquot is a **derived `SampleItem`** off the collected specimen (the case key); a sibling bacterial case, if any, uses the **raw** specimen — so the two workflows take different aliquots of the *same* collected `SampleItem`, which is exactly why TB decontamination here never affects the bacterial case.
