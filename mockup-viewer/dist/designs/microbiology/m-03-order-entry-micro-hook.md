...
- ~~Confirm M-04 §4.9's culture-protocol picker is reachable outside a workflow change (v2.2).~~ **RESOLVED — M-04 §4.9a added in the same changeset.** The concern was real: *Change workflow* requires choosing a workflow type, demands a reclassification reason, and re-instantiates the profile, breakpoint family, organism vocabulary, reflex variant and WHONET flavour — so using it to correct a recipe would be a sledgehammer and, on a case with results, would trip the detach-results warning for nothing. Investigating it also surfaced a **second** gap this spec had created: AC-M03-04 promises a *"Not set — the bench will select a protocol"* state, and M-04 had no way to **set** a protocol that was never set. M-04 §4.9a now provides a narrow **Set / Change protocol** action on the Inoculation section — active Methods within the current `workflow_type` only, one-line reason, Timeline note, no profile side-effects, existing inoculation rows never rewritten, and the incubation clock recomputed against the new maximum from the original inoculation date. M-03 v2.2 therefore ships with a working replacement rather than a hole. See M-04 AC-M04-20…25.
